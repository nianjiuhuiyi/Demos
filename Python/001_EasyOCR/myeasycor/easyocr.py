...
class Reader(object):
    ...
    def detect(self, imgs, min_size=20, text_threshold=0.7, low_text=0.4, \
               link_threshold=0.4, canvas_size=2560, mag_ratio=1., \
               slope_ths=0.1, ycenter_ths=0.5, height_ths=0.5, \
               width_ths=0.5, add_margin=0.1, reformat=True, optimal_num_chars=None):

        text_box_list = get_textbox(self.detector, imgs, canvas_size, mag_ratio, \
                                    text_threshold, link_threshold, low_text, \
                                    False, self.device, optimal_num_chars)
        horizontal_lists, free_lists = group_text_box(text_box_list, slope_ths, \
                                                      ycenter_ths, height_ths, \
                                                      width_ths, add_margin, \
                                                      (optimal_num_chars is None))

        if min_size:
            for k in range(len(horizontal_lists)):
                horizontal_lists[k] = [i for i in horizontal_lists[k] if max(i[1] - i[0], i[3] - i[2]) > min_size]
                free_lists[k] = [i for i in free_lists[k] if
                                 max(diff([c[0] for c in i]), diff([c[1] for c in i])) > min_size]

        return horizontal_lists, free_lists

    def recognize(self, imgs_cv_grey, horizontal_lists=None, free_lists=None, \
        ...
        outputs = []
        for i in range(len(imgs_cv_grey)):
            if (horizontal_lists[i] == None) and (free_lists[i] == None):
                y_max, x_max = imgs_cv_grey[i].shape
                ratio = x_max / y_max
                max_width = int(imgH * ratio)
                crop_img = cv2.resize(imgs_cv_grey[i], (max_width, imgH), interpolation=Image.ANTIALIAS)
                image_list = [([[0, 0], [x_max, 0], [x_max, y_max], [0, y_max]], crop_img)]
            else:
                image_list, max_width = get_image_list(horizontal_lists[i], free_lists[i], imgs_cv_grey[i],
                                                       model_height=imgH)
            ...
            output = [item[1] for item in result] if detail == 0 else result
            outputs.append(output)
        return outputs

    def readtext(self, images, images_grey, decoder='greedy', beamWidth=5, batch_size=1, \
                 workers=0, allowlist=None, blocklist=None, detail=1, \
        ...
        horizontal_lists, free_lists = self.detect(images, min_size, text_threshold, \
        ...
        result = self.recognize(images_grey, horizontal_lists, free_lists,
